...
public class DayTester {
	...
	static void display(Day day) {
		...
		System.out.println("연도 내 잔여 일수:" + day.leftDayYear());
	}
	
	//--- 날짜 변경 ---//
	static void change(Day day) {
		System.out.println("[1]연월일을 변경 [2]연을 변경");
		System.out.println("[4]월을 변경 [4]일을 변경");
		...
		switch(change) {
		case 1: day.set(y, m, d); break;              //y년 m월 d일로 설정
	...
	static void compare(Day day) {
		...
		int comp = day.compareTo(d2);    //int comp = compareTo(day,d2)도 가능
