...
public class Day {
	...
	public Day() {
		...
		this.year = today.get(GregorianCalendar.YEAR);          //연
		this.month = today.get(GregorianCalendar.MONTH)+1;      //월
		this.date = today.get(GregorianCalendar.DATE);          //일
	}
	//--- 생성자(year년1월1일) ---//
	public Day(int year) {
		this.year = year;
	...
	public String toString() {
		...
		return String.format("%04d년%02d월%02d일(%s)", year, month, date, dayOfWeek());
	}
	//--- 연내 경과 일수 ---//
	...
	public static int compare(Day d1, Day d2) {
		...
		return d1.date > d2.date || d1.date < d2.date ? -1 : 0;   //일도 같다.
	}
	//--- 날짜를 하루 뒤로 변경 ---//
	public void succeed() {
